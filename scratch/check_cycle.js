...
async function run() {
  try {
    // Let's load all composition items that are sub-compositions
    const { data: items, error } = await supabase.schema('engenharia')
    ...
    console.log(`Loaded ${items?.length} sub-composition relationships.`);
    ...
    if (hasCycle) {
      ...
      // Let's resolve the IDs to actual codes to show the user
      ...
      console.log("\n✅ No circular references found in sub-composition links.");
    }
