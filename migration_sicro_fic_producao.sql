-- MIGRATION: Adiciona suporte a FIC, Produção da Equipe, Tempo Fixo e Atividades Auxiliares para o SICRO
-- IMPORTANTE: Execute este script no SQL Editor do Supabase antes de reimportar a base SICRO.

ALTER TABLE engenharia.composicoes ADD COLUMN IF NOT EXISTS producao_equipe NUMERIC(15,6) DEFAULT 1;
ALTER TABLE engenharia.composicoes ADD COLUMN IF NOT EXISTS fic_factor NUMERIC(15,6) DEFAULT 0;
ALTER TABLE engenharia.composicoes ADD COLUMN IF NOT EXISTS custo_tempo_fixo NUMERIC(15,6) DEFAULT 0;
ALTER TABLE engenharia.composicoes ADD COLUMN IF NOT EXISTS custo_atividades_auxiliares NUMERIC(15,6) DEFAULT 0;

-- Recriar a função de cálculo de CDU (fn_calcular_cdu_composicao) considerando a nova lógica de FIC e Rateio do SICRO
...
BEGIN
  ...
  LOOP
    IF v_item.insumo_id IS NOT NULL THEN
      ...
      INTO 
        v_item_valor,
        v_item_improd_valor,
        v_item_improd_valor -- Temporário, usando variável existente para economizar DECLARE
      FROM engenharia.insumos 
      WHERE id = v_item.insumo_id;

      -- Verificar se é mão de obra ou equipamento
      -- NOTA: O terceiro parâmetro da query 'tipo' foi lido, vamos checar qual tipo é
      DECLARE
        v_ins_tipo TEXT;
      BEGIN
  ...
  -- Adicionar FIC, Tempo Fixo e Atividades Auxiliares se for SICRO
  IF v_is_sicro THEN
